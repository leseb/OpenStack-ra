...
nova_consoleauth_monitor() {
    ...
    if ! check_binary $OCF_RESKEY_monitor_binary; then 
		...
		CONSOLE_AMQP_CO_CHECK=`"$OCF_RESKEY_monitor_binary" -punt | egrep "$OCF_RESKEY_database_server_port" | egrep "$PID" | egrep -q "ESTABLISHED"`
		rc_amqp=$?
		CONSOLE_DATABASE_CO_CHECK=`"$OCF_RESKEY_monitor_binary" -punt | egrep "$OCF_RESKEY_amqp_server_port" | egrep "$PID" | egrep -q "ESTABLISHED"`
		rc_database=$?
    fi
    if [ $rc_amqp -ne 0 ] || [ $rc_database -ne 0 ]; then
        ocf_log err "Nova ConsoleAuth is not connected to the AMQP server or the database: $rc"
        return $OCF_NOT_RUNNING
    fi
